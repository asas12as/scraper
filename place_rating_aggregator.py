...
def search_places_by_governorate(
    ...
    Uses Google Maps instead of Search because Maps returns structured
    business listings — not articles, listicles, or UI noise.
    """
    ...
    with StealthySession(
        ...
        # Accept cookies once (persisted for all category fetches)
        try:
            session.fetch("https://www.google.com/maps", wait=2000, page_action=accept_cookies)
        except Exception:
            pass
        ...
        for cat in categories:
            ...
            try:
                response = session.fetch(url, wait=5000, page_action=scroll_feed)

                seen = set()
                cat_count = 0
                for article in response.css('[role="article"]'):
                    aria = article.attrib.get("aria-label", "")
                    if aria:
                        name = aria.split("·")[0].strip().rstrip(" ·★")
